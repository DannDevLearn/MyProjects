...
public class CarFleet {
    public static void main(String[] args) {
        ...
        int[] speed = {2,4,1,1,3};
        System.out.println(solution(target, position, speed));

    }
    private static int solution(int target, int[] position, int[] speed){
        ...
        for (int pos:position){
            double timToFinish = (target-pos) / map.get((double) pos);

            while (!stack.isEmpty() && timToFinish>=stack.peek())
                stack.pop();

            stack.push(timToFinish);
        }
